...
# ==============================================================================
# CSS: TEMA ESCURO NO APLICATIVO + TABELAS COM FUNDO BRANCO E CORES
# ==============================================================================
st.markdown(
    """
<style>
    /* 1. App Geral: Tema Escuro Original */
    .stApp {
    ...
    * {
    ...
    /* 3. Abas Superiores Legíveis (100% Visíveis com Tema Escuro) */
    ...
    .badge-ok { background-color: rgba(46, 160, 67, 0.2) !important; color: #3fb950 !important; border: 1px solid #2ea043 !important; }
    .badge-warn { background-color: rgba(210, 153, 34, 0.2) !important; color: #d29922 !important; border: 1px solid #d29922 !important; }
    .badge-alert { background-color: rgba(248, 81, 73, 0.2) !important; color: #ff7b72 !important; border: 1px solid #f85149 !important; }

    /* 5. TABELAS (DATAFRAMES): FUNDO BRANCO, TEXTO PRETO E BORDAS CLARAS */
    ...
    /* Barra de ferramentas interna do Glide-Data-Grid */
...
# ==============================================================================
# CONFIGURAÇÃO DE DIRETÓRIOS E ROTAS ONEDRIVE / DRIVE G:
# ==============================================================================
...
# Localização automática do OneDrive corporativo
...
# Localização do Romaneio no Drive G:
...
# ==============================================================================
# SINCRONIZADOR ONEDRIVE / DRIVE G:
# ==============================================================================
def sincronizar_pastas_locais():
    ...
    if os.path.exists(CAMINHO_PCP):
        try:
            ...
            if meta.get("mtime_pcp") != mtime_pcp:
            ...
            st.sidebar.error(f"Erro ao ler Alinhamento PCP: {e}")

    # 2. MACRO PRODUCAO COM QTD.xlsm
    if os.path.exists(CAMINHO_MACRO_OP):
        try:
            mtime_op = os.path.getmtime(CAMINHO_MACRO_OP)
            if meta.get("mtime_op") != mtime_op:
                xl_op, tmp_op = abrir_excel_seguro_copia(CAMINHO_MACRO_OP)
                try:
                    for s in xl_op.sheet_names:
                        s_norm = normalizar_texto(s)
                        if s_norm.startswith("EXTRA") or "MARCO" in s_norm:
                            df_o = ler_excel_seguro(tmp_op, sheet_name=s, header=9).astype(str)
            ...
            st.sidebar.error(f"Erro ao ler Macro Produção: {e}")

    # 3. Romaneio TOTVS
    if os.path.exists(CAMINHO_ROMANEIO):
        try:
            mtime_rom = os.path.getmtime(CAMINHO_ROMANEIO)
            if meta.get("mtime_rom") != mtime_rom:
            ...
            st.sidebar.error(f"Erro ao ler Romaneio Drive G: {e}")
...
# ==============================================================================
# FORMATAÇÃO VISUAL DOS STATUS NAS TABELAS (CORES E ÍCONES)
# ==============================================================================
def formatar_coluna_status_visual(df, col_status):
    """Adiciona emojis e tags coloridas na coluna de status sem usar Styler pesado."""
    ...
    def aplicar_tag(val):
        s = str(val).strip()
        if "100%" in s or ("Entregue" in s and "Parcial" not in s and "Falta" not in s):
            return f"🟢 {s}"
        elif "Falta" in s or "Gargalo" in s or "Produção" in s:
            return f"🔴 {s}"
        elif "Não Enviado" in s or "Parcial" in s or "Aguardando Despacho" in s:
            return f"🟡 {s}"
        elif "Tratamento" in s or "Fornecedor" in s or "SC" in s:
            return f"🔵 {s}"
        return s

    df_fmt[col_status] = df_fmt[col_status].apply(aplicar_tag)
    return df_fmt
...
def processar_todas_as_bases(mtimes, pasta_base=STORAGE_DIR):
    ...
    if not df_comp_raw.empty:
        col_obs_comp = buscar_col_flex(df_comp_raw, ["OBSERVAÇÃO", "OBSERVAÇÕES", "OBSERVACAO", "OBSERVACOES", "OBS", "TAT", "PROJETO", "LOTE"])
    ...
    if not df_cruz_obs.empty:
        ...
        for col_str in ["Data_Fabricacao", "Mes_Ano", "Doc_Romaneio", "Data_Envio", "NF_Retorno", "Data_Retorno", "Fornecedor_Tratamento"]:
            if col_str not in df_cruz_obs.columns:
                df_cruz_obs[col_str] = "-"
        ...
        def calc_obs(r):
            dt = str(r.get("Data_Fabricacao", ""))
            ma = str(r.get("Mes_Ano", ""))
            if "2025" in dt or "2025" in ma:
                if r.get("Ret_Pintura", 0) > 0:
                    return "Ano 2025 (Com Romaneio)"
                return "Ano 2025 (Sem controle de romaneio)"
            return "-"

        def calc_status(r):
            cod = str(r.get("COD_PECA", "")).strip()
            dt_fab = str(r.get("Data_Fabricacao", ""))
            mes_ano = str(r.get("Mes_Ano", ""))
            is_2025 = ("2025" in dt_fab) or ("2025" in mes_ano)

            tem_trat_lote = (r["Env_Pintura"] > 0) or (r["Ret_Pintura"] > 0) or (str(r["Fornecedor_Tratamento"]).strip() not in ["-", ""])
            tem_trat = tem_trat_lote or (cod in pecas_com_tratamento)

            if is_2025 and r["Ret_Pintura"] == 0:
                if r["Qtd_Fabr"] >= r["Qtd_OP"] and r["Qtd_OP"] > 0:
                    return "Fabricado em 2025 (Sem Romaneio)"
                elif r["Qtd_Fabr"] > 0:
                    return "Fabricação Parcial (2025)"
                else:
                    return "Aguardando Produção (2025)"

            if tem_trat:
                if r["Ret_Pintura"] >= r["Qtd_OP"] and r["Qtd_OP"] > 0 and r["Saldo_Pendente_Pintura"] == 0 and r["Falta_Fabricar"] == 0:
                    return "100% Entregue"
                elif r["Falta_Fabricar"] > 0:
                    return "Falta Fabricar Internamente"
                elif r["Saldo_Pendente_Pintura"] > 0:
                    return "Em Tratamento Externo"
                elif r["Aguardando_Envio"] > 0:
                    return "Fabricado (Aguardando Envio)"
                ...
                if r["Qtd_Fabr"] >= r["Qtd_OP"] and r["Qtd_OP"] > 0:
                    return "100% Entregue"
                elif r["Falta_Fabricar"] > 0 and r["Qtd_Fabr"] > 0:
                    return "Fabricação Parcial"
                elif r["Falta_Fabricar"] > 0:
                    return "Falta Fabricar Internamente"
                else:
                    return "Aguardando Produção"
        ...
        df_cruz_obs["Observacao"] = df_cruz_obs.apply(calc_obs, axis=1)
...
with st.sidebar:
    ...
    st.markdown("### 🔄 Sincronização Local")
    if st.button("🔄 Sincronizar Pastas Locais", use_container_width=True):
        with st.spinner("Lendo planilhas do OneDrive e Drive G:..."):
            alterou = sincronizar_pastas_locais()
            if alterou:
                st.success("Bases atualizadas com sucesso!")
            else:
                st.info("Nenhuma modificação detectada nas planilhas.")
            st.rerun()

    st.divider()

    with st.expander("📥 Upload Manual de Arquivos", expanded=False):
        st.caption("Use esta área para subir as planilhas se estiver fora da rede/Drive:")
            ...
            try:
                ...
                if not f.name.endswith(".csv"):
                    ...
                    for s_name in xl.sheet_names:
                        s_upper = normalizar_texto(s_name)
                        caminho_salvar = None
                        pular_linhas = 0

                        if "SOLICITA" in s_upper:
                            caminho_salvar = caminho_sc_pqt
                            pular_linhas = 0
                        elif "ENTREGAS-ATUALIZADO" in s_upper or "PEDIDO" in s_upper:
                            caminho_salvar = caminho_comp_pqt
                            pular_linhas = 0
                        elif s_upper.startswith("EXTRA") or "MARCO" in s_upper or "OP" in f_upper:
                            caminho_salvar = caminho_op_pqt
                            pular_linhas = 9
                        elif "ROMANEIO" in f_upper or "ROMANEIO" in s_upper:
                            caminho_salvar = caminho_rom_pqt
                            pular_linhas = 0

                        if caminho_salvar:
                            df_completo = ler_excel_seguro(io.BytesIO(file_bytes), sheet_name=s_name, header=pular_linhas)
                            df_completo = df_completo.loc[:, ~df_completo.columns.str.contains(r"^Unnamed", na=False)]
...
if not os.path.exists(caminho_op_pqt) and not os.path.exists(caminho_comp_pqt):
...
if df_op_raw.empty and df_rom_raw.empty and df_comp_raw.empty and df_sc_raw.empty:
    st.info("👆 Nenhuma base carregada ainda. Clique em **'🔄 Sincronizar Pastas Locais'** na barra lateral ou envie os arquivos.")
    st.stop()
...
with col_multisel_obs:
    sel_obs_global = st.multiselect(
        ...
        placeholder="Digite código ou modelo (Ex: TAT, HILUX, RANGER, COROLLA)...",
    )
...
with tab_metalicos_falta:
    if not df_unificado_global.empty:
        df_faltas = df_unificado_global[df_unificado_global["Status_Unificado"] != "100% Entregue"].copy()
        if not df_faltas.empty:
            df_view_faltas = df_faltas[colunas_view_unificada].rename(columns=nomes_colunas_unificadas)
            df_view_faltas = formatar_coluna_status_visual(df_view_faltas, "Status do Item")
            ...
            with c_btns_tot:
                ...
                with cb2:
                    st.download_button("📥 Exportar (.xlsx)", data=gerar_excel_tabela(df_view_faltas, "Faltas_Metalicos"), file_name="Faltas_Metalicos.xlsx")
            
            st.dataframe(df_view_faltas, use_container_width=True, hide_index=True, height=650)
        else:
            st.success("🎉 Nenhuma falta pendente! Todos os itens estão prontos.")
    else:
...
with tab_metalicos_total:
    if not df_unificado_global.empty:
        c_origem_tot, _ = st.columns([1.5, 2])
        with c_origem_tot:
        ...
        df_view = formatar_coluna_status_visual(df_view, "Status do Item")
        ...
        with c_btns_tot:
            ...
            with cb2:
                st.download_button("📥 Exportar (.xlsx)", data=gerar_excel_tabela(df_view, "Metalicos_Geral"), file_name="Metalicos_Visao_Geral.xlsx")
...
with tab_fabricadas:
    if not df_trabalho.empty:
        ...
        if not df_fab_feitas.empty:
            ...
            df_view_fab = formatar_coluna_status_visual(df_view_fab, "Status Atual")

            c_tit, c_btns = st.columns([3, 2])
            with c_tit:
                st.subheader(f"🏭 Peças Fabricadas ({len(df_fab_feitas)} itens | {tot_fab:,} peças)")
            with c_btns:
                cb1, cb2 = st.columns(2)
                with cb1:
                    renderizar_botao_copiar_tabela(df_view_fab, "btn_cp_fabricadas")
                with cb2:
                    st.download_button("📥 Exportar (.xlsx)", data=gerar_excel_tabela(df_view_fab, "Pecas_Fabricadas"), file_name="Pecas_Fabricadas.xlsx")
...
with tab_retornadas:
    if not df_trabalho.empty:
        ...
        if not df_p2.empty:
            ...
            df_view_ret = formatar_coluna_status_visual(df_view_ret, "Status")

            c_tit, c_btns = st.columns([3, 2])
            with c_tit:
                st.subheader(f"✅ Peças Retornadas ({len(df_p2)} itens | {tot_ret:,} peças)")
            with c_btns:
                cb1, cb2 = st.columns(2)
                with cb1:
                    renderizar_botao_copiar_tabela(df_view_ret, "btn_cp_retornadas")
                with cb2:
                    st.download_button("📥 Exportar (.xlsx)", data=gerar_excel_tabela(df_p2, "Pecas_Retornadas"), file_name="Pecas_Retornadas.xlsx")
            st.dataframe(df_view_ret, use_container_width=True, hide_index=True, height=650)
        else:
            st.info("Nenhuma peça com retorno registrado.")
    else:
        st.info("Nenhum dado encontrado.")
...
with tab_pend_trat:
    if not df_trabalho.empty:
        ...
        if not df_p1.empty:
            ...
            c_tit, c_btns = st.columns([3, 2])
            with c_tit:
                st.subheader(f"🚨 Falta Retorno de Tratamento ({len(df_p1)} itens | {saldo_rua:,} peças)")
            with c_btns:
                cb1, cb2 = st.columns(2)
                with cb1:
                    renderizar_botao_copiar_tabela(df_view_falta_ret, "btn_cp_falta_ret")
                with cb2:
                    st.download_button("📥 Exportar (.xlsx)", data=gerar_excel_tabela(df_p1, "Falta_Retorno_Tratamento"), file_name="Falta_Retorno_Tratamento.xlsx")
            st.dataframe(df_view_falta_ret, use_container_width=True, hide_index=True, height=650)
        else:
            st.success("🎉 Nenhuma peça pendente de retorno!")
    else:
        st.info("Nenhum dado encontrado.")
...
with tab_aguard_envio:
    if not df_trabalho.empty:
        ...
        if not df_p3.empty:
            ...
            c_tit, c_btns = st.columns([3, 2])
            with c_tit:
                st.subheader(f"🚚 Peças Aguardando Envio ({len(df_p3)} itens | {falta_env:,} peças)")
            with c_btns:
                cb1, cb2 = st.columns(2)
                with cb1:
                    renderizar_botao_copiar_tabela(df_view_aguard, "btn_cp_aguardando")
                with cb2:
                    st.download_button("📥 Exportar (.xlsx)", data=gerar_excel_tabela(df_p3, "Aguardando_Envio"), file_name="Fabricadas_Aguardando_Envio.xlsx")
...
with tab_falta_fab:
    if not df_trabalho.empty:
        ...
        if not df_p4.empty:
            ...
            c_tit, c_btns = st.columns([3, 2])
            with c_tit:
                st.subheader(f"⚙️ Peças que Faltam Fabricar ({len(df_p4)} itens | {falta_fab:,} peças)")
            with c_btns:
                cb1, cb2 = st.columns(2)
                with cb1:
                    renderizar_botao_copiar_tabela(df_view_falta_fab, "btn_cp_falta_fab")
                with cb2:
                    st.download_button("📥 Exportar (.xlsx)", data=gerar_excel_tabela(df_p4, "Falta_Fabricar"), file_name="Falta_Fabricar.xlsx")
...
with tab_compras:
    if not df_comp_trabalho.empty:
        ...
        with col_st_f:
            ...
            sel_st_comp = st.multiselect("Filtrar por Status:", status_comp, default=status_pendentes)
        ...
        df_view_compras = formatar_coluna_status_visual(df_view_compras, "Status")

        c_tit, c_btns = st.columns([3, 2])
        with c_tit:
            st.subheader(f"📦 Compras Externas ({len(df_comp_trabalho)} itens | {tot_entregue:,} de {tot_comprado:,} entregues)")
        with c_btns:
            cb1, cb2 = st.columns(2)
            with cb1:
                renderizar_botao_copiar_tabela(df_view_compras, "btn_cp_compras")
            with cb2:
                st.download_button("📥 Exportar (.xlsx)", data=gerar_excel_tabela(df_comp_trabalho, "Compras_Externas"), file_name="Compras_Externas.xlsx")
        st.dataframe(df_view_compras, use_container_width=True, hide_index=True, height=650)
    else:
        st.info("Nenhum item de Compras encontrado.")
...
with tab_sc:
    if not df_sc_trabalho.empty:
        ...
        with col_ped_f:
            ...
            sel_ped_sc = st.multiselect("Filtrar por Pedido:", pedidos_sc, default=[])
        ...
        with c_btns:
            ...
            with cb2:
                st.download_button("📥 Exportar (.xlsx)", data=gerar_excel_tabela(df_sc_trabalho, "SC_Em_Aberto"), file_name="Solicitacoes_Compras_Aberto.xlsx")
        st.dataframe(df_view_sc, use_container_width=True, hide_index=True, height=650)
    else:
        st.info("Nenhuma Solicitação de Compras em aberto encontrada.")

# ------------------------------------------------------------------------------
# 11. ABA BASES DE DADOS BRUTAS
# ------------------------------------------------------------------------------
with tab_dados_brutos:
    st.markdown("### 📂 Visualizador de Bases Brutas")
    tipo_base = st.radio(
    ...
    if tipo_base == "📑 OP Fabricação":
        if not df_op_raw.empty:
            ...
            df_mostrar = df_op_raw[df_op_raw["Produto"].astype(str).str.contains(busca_cod, case=False, na=False)] if busca_cod and "Produto" in df_op_raw.columns else df_op_raw
        ...
        if not df_rom_raw.empty:
            ...
            df_mostrar = df_rom_raw[df_rom_raw["PRODUTO"].astype(str).str.contains(busca_cod, case=False, na=False)] if busca_cod and "PRODUTO" in df_rom_raw.columns else df_rom_raw
            st.dataframe(df_mostrar, use_container_width=True, hide_index=True, height=600)
        else:
